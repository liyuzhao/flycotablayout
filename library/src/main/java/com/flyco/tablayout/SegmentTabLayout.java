...
public class SegmentTabLayout extends FrameLayout implements ValueAnimator.AnimatorUpdateListener {
    ...
    public void setTabData(Integer[] drawableIds, int baseLineId) {
        if (drawableIds == null || drawableIds.length == 0) {
            throw new IllegalStateException("drawableIds can not be NULL or EMPTY !");
        }
    ...
    /** 更新数据 */
    public void notifyDataSetChanged(boolean isText) {
        mTabsContainer.removeAllViews();
        if (isText) {
            this.mTabCount = mTitles.length;
        } else {
            this.mTabCount = drawableIds.length;
        }
        View tabView;
        for (int i = 0; i < mTabCount; i++) {
            tabView = View.inflate(mContext, R.layout.layout_tab_segment, null);
            tabView.setTag(i);
            addTab(i, tabView, isText);
        }

        if (isText) updateTabStyles();
    }

    /** 创建并添加tab */
    private void addTab(final int position, View tabView, boolean isText) {
        TextView tv_tab_title = (TextView) tabView.findViewById(R.id.tv_tab_title);
        tv_tab_title.setGravity(Gravity.CENTER);
        if (isText) {
            tv_tab_title.setText(mTitles[position]);
        } else {
            tv_tab_title.setBackgroundResource(drawableIds[position]);
        }
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            if (baseLineId != 0) {
                Drawable drawable = getResources().getDrawable(baseLineId);
                drawable.setBounds(0, 0, 60, 12);
                if (isSelect) {
                    tab_title.setCompoundDrawables(null, null, null, drawable);
                } else {
